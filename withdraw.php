...
if($_SERVER['REQUEST_METHOD']=="POST"){
    ...
    if(!is_numeric($amount) || floatval($amount) > floatval($_SESSION['bal'])){
        $err = "Not Enough Balance !!<br>";
    }
    if(strlen($refc) > 12){
        $err .= "Referal Code can be atmost 12 Characters !!";
    }

    if(empty($err) && !$_SESSION['withdrawn']){
        $sql = "SELECT id,balance,pwd FROM nrracc_primary WHERE accnum = '$accnum'";
        $row = mysqli_fetch_assoc(mysqli_query($conn, $sql));

        $db_pwd = $row['pwd'];
        if(password_verify($pwd, $db_pwd) && !$_SESSION['withdrawn']){
            ...
            $sql = "INSERT INTO accstatements (accnum, amount, type, des, ref, date,datetime, isapproved) VALUES('$accnum','$amount','W','$description','$refc','$today','$now',1)";
            mysqli_query($conn, $sql);
            
            $_SESSION['withdrawn'] = true;
        }
        else{
...
<head>
    ...
    <title>Money Withdrawals : NRR Bank</title>
    <style>
        body {
    ...
    <div class="container signin my-5">
        <div class="card">
            <div class="card-body">
                ...
                <h1 class="card-title title text-center">Withdraw Money <i class="fas fa-credit-card"></i></h1>
                <h6 class="card-subtitle mb-2 text-muted text-center">Withdraw Money upto your Balance Limit</h6>
                <?php
                    if(!empty($err)){
                        echo '<div class="alert alert-danger text-center mt-3">'.$err.'</div>';
                    }
                    if($_SESSION['withdrawn']){
                        echo '<div class="alert alert-success text-center mt-3"><i class="fas fa-rupee-sign"></i> '.number_format($amount,2).'/- Withdrawn Succesfully !!</div>';
                    }
                ?>
                ...
                ">
                    ...
                    <div class="form-group row my-3">
                        ...
                        <input type="text" class="form-control col-8" name="ref" aria-describedby="emailHelp"
