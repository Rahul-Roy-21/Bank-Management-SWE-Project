...
if($_SERVER["REQUEST_METHOD"] == "POST"){
    ...
    if(empty($err) && strlen($npwd) < 5){
       $err = "New Password Must Have Atleast 5 characters !!"; 
    }
    ...
    if(empty($err) && !$_SESSION['changepwd']){
        ...
        if(password_verify($pwd, $row['pwd'])){
            ...
            $res = sendMail($_SESSION['name'], $_SESSION['email'], $_SESSION['otp'], 'CPWDVERIFY');
...
<head>
    ...
    <title>Password Updation: NRR Bank</title>
    <style>
        body {
    ...
    <div class="container signin my-5">
        <div class="card">
            <div class="card-body">
                ...
                <h6 class="card-subtitle mb-2 text-muted text-center">Change your Unique Password to Keep your Account Secure</h6>

                <?php
                    if(!empty($err)){
                        echo '<div class="alert alert-danger text-center mt-3">'.$err.'</div>';
                    }
                    if($_SESSION['changepwd'] && $_SESSION['verified']){
                        echo '<div class="alert alert-success text-center mt-3"><b>Your Password is Successfully Updated !!</b></div>';
                    }
                ?>
